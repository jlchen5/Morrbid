...
# 保存数据
saveRDS(heartok, file = "./scRNAseq3_MouseHeart/heartok.rds")
...
#plot_data2 <- plot_data[plot_data[, 3] != 0, ]
...
ggplot(plot_data, aes(x = group, y = expression, fill = group)) +
  ...
  facet_wrap(~ gene, ncol = 3, scales = "free_y") +  # 按基因分面，垂直排列
  labs(title = "Gene Expression", y = "Log-Normalized Expression") +
  theme_bw() 
